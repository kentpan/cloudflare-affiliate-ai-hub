...
import { ensureDataRootSync, readJsonSync, writeJsonSync, dataPath } from "./node-data-writer";
...
export function readConfig(): CustomizationConfig {
  ensureDataRootSync();
  const c = readJsonSync<CustomizationConfig>(getConfigPath());
...
export function writeConfig(c: CustomizationConfig): void {
  ...
  writeJsonSync(getConfigPath(), normalized);
}
