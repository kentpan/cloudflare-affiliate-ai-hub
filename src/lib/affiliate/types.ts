...
export type Platform = "amazon" | "taobao" | "jd" | "google" | "pdd" | "douyin" | "kuaishou" | "xhs";

export const PLATFORMS: Platform[] = ["amazon", "taobao", "jd", "google", "pdd", "douyin", "kuaishou", "xhs"];
...
// 淘宝客/京东/拼多多/抖音/快手/小红书 are Chinese platforms priced in CNY (¥).
...
export interface DataIndex {
  ...
  dates: string[];
  platforms: Platform[];
  // 动态平台计数:每个日期下的各平台选品数 + total
  // 向后兼容:旧数据可能只有 amazon/taobao/jd/google,新数据可能包含 pdd/douyin/kuaishou/xhs
  totals: Record<string, Partial<Record<Platform, number>> & { total: number }>;
}
