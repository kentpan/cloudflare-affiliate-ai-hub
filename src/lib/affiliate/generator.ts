...
export async function generateDaily(opts: GenerateOptions = {}): Promise<GenerateResult> {
  ...
  const adapters = [
    ...
    { name: "pdd" as const, adapter: new PddAdapter() },
    { name: "douyin" as const, adapter: new DouyinAdapter() },
    { name: "kuaishou" as const, adapter: new KuaishouAdapter() },
    { name: "xhs" as const, adapter: new XhsAdapter() },
  ];

  const platformResults = PLATFORMS.reduce((acc, p) => {
    acc[p] = { raw: 0, picked: 0, picks: [] };
    return acc;
  }, {} as Record<Platform, { raw: number; picked: number; picks: PickedProduct[] }>);
  ...
  const all: PickedProduct[] = PLATFORMS.flatMap((p) => platformResults[p].picks).sort((a, b) => b.score - a.score);
  ...
  // 动态构建 totals:每个平台的 picked 数 + total(支持向后兼容)
  const totalsEntry: Partial<Record<Platform, number>> & { total: number } = { total: all.length };
  for (const p of PLATFORMS) {
    totalsEntry[p] = platformResults[p].picked;
  }
  index.totals[date] = totalsEntry;
