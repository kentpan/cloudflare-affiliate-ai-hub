...
import { ensureDateDir, readJson, writeJson, dataPath } from "./data-writer";
import type {
  DataIndex,
  DailySummary,
  PickedProduct,
...
export async function generateDaily(opts: GenerateOptions = {}): Promise<GenerateResult> {
  ...
  for (const { name, adapter } of adapters) {
    try {
      ...
      writeJson(path.join(dir, `${name}.json`), picks);
    } catch (e) {
  ...
  writeJson(path.join(dir, "dimensions", "by-platform.json"), {
    dimension: "platform",
    groups: byPlatform(all),
  });
  writeJson(path.join(dir, "dimensions", "by-category.json"), {
    dimension: "category",
    groups: byCategory(all),
  });
  writeJson(path.join(dir, "dimensions", "by-commission.json"), {
    dimension: "commission",
    groups: byCommission(all),
  });
  writeJson(path.join(dir, "dimensions", "by-price-range.json"), {
  ...
  const index: DataIndex = readJson<DataIndex>(indexPath) ?? {
  ...
  const log = readJson<GenerationLogEntry[]>(logPath) ?? [];
  ...
  log.unshift(entry);
  writeJson(logPath, log.slice(0, 100));
