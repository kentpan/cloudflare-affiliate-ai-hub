...
export const getIndex = () => fetchJson<DataIndex>("/api/data/index");
export const getDates = () =>
  fetchJson<{ dates: string[] }>("/api/data/dates");
export const getSummary = (date: string) =>
  fetchJson<DailySummary>(`/api/data/summary?date=${date}`);
export const getDimension = (date: string, name: DimensionName) =>
  fetchJson<DimensionFile>(`/api/data/dimension?date=${date}&name=${name}`);
export const getPlatform = (date: string, platform: Platform) =>
  fetchJson<{ date: string; platform: Platform; items: PickedProduct[] }>(
    `/api/data/platform?date=${date}&platform=${platform}`,
  );
export const getLogs = () =>
  fetchJson<{ logs: GenerationLogEntry[] }>("/api/logs");
